...
import { useState, useContext, useEffect, useRef, useCallback, useMemo, memo } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import AuthContext from '../context/AuthContext';
import { motion, AnimatePresence } from 'framer-motion';
import {
...
const OTP_LENGTH = 6;
const OTP_INITIAL = Array.from({ length: OTP_LENGTH }, () => '');
...
const getPasswordStrength = (password) => {
  ...
  if (score <= 2) return { score, label: 'Weak', color: '#ff7c7c', percent: (score / 6) * 100 };
  if (score <= 4) return { score, label: 'Medium', color: '#d8b5ff', percent: (score / 6) * 100 };
  return { score, label: 'Strong', color: '#5eead4', percent: (score / 6) * 100 };
};
...
const Register = memo(() => {
  ...
  }, [acceptedTerms, formData.name, formData.email, formData.password, register]);
  ...
  }, [otp, verifyOtp, formData.name, formData.email, formData.password, navigate]);
      ...
      >
        <motion.div variants={itemVariants} className="text-center mb-8">
          ...
          <p className="mt-3 text-sm text-slate-400 sm:text-base">
            {step === 1 ? 'Start your financial journey' : `Code sent to ${formData.email}`}
          </p>
        </motion.div>
        ...
        >
          ...
          {step === 1 ? (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <label className="flex items-start gap-3">
                <input
                  ...
                  className="mt-0.5 h-4 w-4 rounded border-slate-700 bg-slate-900 text-emerald-500 focus:ring-emerald-400"
                />
            ...
            <form onSubmit={handleOtpSubmit} className="space-y-5">
              <div className="text-center">
                <div className="inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium bg-slate-800/50 text-slate-300">
                  <Smartphone size={14} /> Expires: {formatTime(timeLeft)}
                </div>
              </div>

              <div className="space-y-3">
                <label className="block text-center text-sm font-semibold text-slate-200">
                  Enter 6-digit code
                </label>
                ...
                <div className="flex justify-center gap-2">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      disabled={loading}
                      className="h-12 w-12 rounded-xl border border-slate-800 bg-slate-950/90 text-center text-xl font-bold text-slate-100 outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20 sm:h-14 sm:w-14"
                    />
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className="flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-400 px-6 py-3 text-sm font-semibold text-slate-950 transition hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {loading ? (
              ...
              <div className="space-y-2 text-center">
                <button
                  ...
                  className="text-sm text-slate-400 transition-all duration-300 hover:text-emerald-400 disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="block w-full text-center text-sm text-slate-400 transition hover:text-emerald-400"
                >
                  ← Change email
                </button>
              </div>
          ...
          {step === 1 && (
            <>
              <div className="relative my-6 text-center text-xs text-slate-500">
                ...
                <span className="relative bg-slate-900 px-3">Or sign up with</span>
              </div>
          ...
          <div className="mt-6 text-center text-sm text-slate-400">
            ...
            <Link className="font-semibold text-emerald-400 transition hover:text-cyan-400 hover:underline" to="/login">
              Sign in
            </Link>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="mt-6 flex items-center justify-center gap-2 text-xs text-slate-500">
          <Shield size={12} /> Protected with 256-bit encryption <Sparkles size={12} className="text-emerald-400" />
        </motion.div>
      </motion.div>
    </div>
  );
