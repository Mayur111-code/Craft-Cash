...
// ─── Static variants (defined outside to avoid recreation) ───────────────────
const pageVariants = {
...
const InstallBanner = memo(({ onInstall, onDismiss }) => (
    <motion.div
        ...
        animate="visible"
        exit="exit"
        className="fixed bottom-0 left-0 right-0 z-[100] bg-[var(--card-bg)]
            border-t border-[var(--border-color)] shadow-2xl p-4"
    >
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)]
            ...
            <div className="flex gap-3">
                <button
                    onClick={onDismiss}
                    className="px-4 py-2 rounded-full bg-[var(--element-bg)]
                        text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors duration-200"
                >
                    Maybe Later
                </button>
                <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    className="px-6 py-2 rounded-full bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)]
...
// ─── Route loading bar (replaces heavy overlay) ───────────────────────────────
...
const Layout = ({ children }) => {
    ...
    // Route change: scroll + loading bar
    useEffect(() => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
        setIsLoading(true);
    ...
    return (
        <div className="layout-wrapper min-h-screen flex flex-col bg-[var(--bg-color)]">
            ...
            {/* Loading bar */}
            <AnimatePresence>{isLoading && <LoadingBar key="loading-bar" />}</AnimatePresence>
            ...
            <AnimatePresence>
                {!isOnline && (
                    <motion.div
                        ...
                        animate="visible"
                        exit="exit"
                        className="fixed top-0 left-0 right-0 z-[200] bg-red-500/90
                            backdrop-blur-md text-white py-2 px-4 text-center text-sm font-medium"
                    >
                        <div className="flex items-center justify-center gap-2">
                            <WifiOff size={16} />
                            You are offline. Some features may be unavailable.
