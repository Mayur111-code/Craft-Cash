...
const dropdownVariants = {
  ...
  exit: { opacity: 0, y: -8, scale: 0.98, pointerEvents: 'none' },
};

const Navbar = memo(({ installPrompt, onInstallClick }) => {
  const { user, logout } = useContext(AuthContext);
  const location = useLocation();
  ...
  useEffect(() => {
    ...
    if (isDarkMode) {
      ...
      root.style.setProperty('--card-bg', 'rgba(20, 32, 56, 0.98)');
      root.style.setProperty('--border-color', 'rgba(255, 255, 255, 0.08)');
      root.style.setProperty('--element-bg', 'rgba(255, 255, 255, 0.04)');
    } else {
      root.style.setProperty('--primary', '#4f46e5');
      root.style.setProperty('--secondary', '#7c3aed');
      root.style.setProperty('--bg-color', '#f8fafc');
      ...
      root.style.setProperty('--card-bg', '#ffffff');
      root.style.setProperty('--border-color', '#e2e8f0');
      root.style.setProperty('--element-bg', '#f1f5f9');
    }
  }, [isDarkMode]);
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            <Link to="/" className="flex items-center gap-2 group">
              ...
              <div className="hidden sm:block">
                ...
                <span className="text-[10px] lg:text-xs text-[var(--text-muted)] block -mt-1">
                  Smart Finance
                </span>
              </div>
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon size={16} className={active ? 'text-white' : ''} />
                    {item.label}
                    {active && <span className="absolute inset-0 rounded-full bg-white/20 animate-pulse" />}
                  </Link>
                );
              })}
            </div>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="p-2 rounded-full bg-[var(--element-bg)] border border-[var(--border-color)] transition-all duration-300"
                aria-label="Toggle theme"
              >
                ...
                <AnimatePresence>
                  {showUserMenu && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-64 rounded-xl bg-[var(--element-bg)]
                        border border-[var(--border-color)] shadow-2xl overflow-hidden"
                    >
                      <div className="p-3 border-b border-[var(--border-color)]">
                        <div className="font-semibold text-[var(--text-main)]">{user?.name}</div>
                      ...
                      <div className="p-2">
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-red-500 hover:bg-red-500/10 transition-all duration-300"
                        >
                          <LogOut size={16} />
                          <span className="text-sm font-medium">Logout</span>
                        </button>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="md:hidden fixed left-0 right-0 bg-[var(--bg-color)]/95 backdrop-blur-xl border-t border-[var(--border-color)] shadow-2xl"
            >
              <div className="p-4 space-y-2 max-h-[80vh] overflow-y-auto">
                <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-[var(--primary)]/10 to-[var(--secondary)]/10 mb-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[var(--primary)] to-[var(--secondary)]
                ...
                {installPrompt && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl
                      bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] text-white font-semibold mt-4"
                  >
                    <Download size={18} />
                    Install App
                  </motion.button>
